    ...
    <div className="max-w-md mx-auto bg-white min-h-screen">
          ...
          cartItems.map((item) => (
            <div key={item.id} className="flex items-center space-x-3 bg-gray-50 p-3 rounded-lg">
              ...
              <div className="flex flex-col items-end space-y-2">
                ...
                <span className="text-xs text-gray-900">{formatRupiah(item.price * item.quantity)}</span>
              </div>
            </div>
          ))
        )}
      ...
      {cartItems.length > 0 && (
        <div className="px-5 pb-4 rounded-lg bg-gray-50">
          ...
          <button disabled={isLoading} onClick={handleCheckout} className="w-full mt-6 bg-[#FFA733] text-black py-4 rounded-full font-semibold hover:bg-[#FFA733]/90 transition-colors">
            {isLoading ? (
              <span className="flex items-center justify-center gap-2">
                <svg className="w-4 h-4 animate-spin text-black" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
