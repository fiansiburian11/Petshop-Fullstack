...
import clsx from "clsx"; // opsional, jika kamu pakai
...
export default function SuksesPage() {
  ...
  useEffect(() => {
    const fetchDetail = async () => {
      ...
      try {
        const res = await axiosInstance.get("/pesanan/sukses", {
  ...
  return (
    <div className="max-w-xl mx-auto p-4 mt-10  rounded ">
      ...
      <div>
        <h1 className="text-center mt-10">Hubungi Kami </h1>
        {/* WhatsApp Box */}
        <div className="flex space-x-5 items-center justify-center mt-2 ">
          {sosmed.map((item) => (
            <Link href={item.link} target="_blank" rel="noopener noreferrer" key={item.id} className=" hover:bg-gray-200 rounded-2xl shadow-md flex flex-col items-center p-2 border">
